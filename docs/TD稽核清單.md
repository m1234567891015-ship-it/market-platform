...
| TD-20 | 低 | 測試/交付 | `verify_release_integrity.py` 假設工作目錄含資料庫,查表前未先建表,乾淨交付包解壓後執行會噴 `no such table`(外部技術回饋發現,記帳補登,本輪不修) | verify_release_integrity.py(直接 `SELECT COUNT(*) FROM option_chain_snapshot` 等,無前置 `CREATE TABLE IF NOT EXISTS`) | 排除 SQLite 是刻意的設計取捨(CLAUDE.md 明訂資料與程式碼分離),並非缺陷;此腳本目前是「工作目錄完整性檢查」,尚不是「乾淨交付包完整性檢查」,對「已排除 60MB SQLite 的乾淨交付包」解壓後直接執行會噴 `sqlite3.OperationalError: no such table: option_chain_snapshot` | 應區分兩種模式:(a) 工作目錄完整性檢查(現況);(b) 交付包完整性檢查(先建 schema 或跳過資料表查詢) | 待評估 | 20 |
